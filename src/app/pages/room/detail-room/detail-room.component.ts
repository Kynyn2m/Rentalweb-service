  ...
  implements OnInit, AfterViewInit, AmenityCounts
{
  ...
  ngOnInit(): void {
    ...
    if (roomsId) {
      this.goToDetailsRoom1(roomsId); // Fetch rooms details
  ...
  ngAfterViewInit(): void {
    ...
    if (roomsId) {
      this.goToDetailsRoom1(roomsId);
    }
  }

  goToDetailsRoom1(id: number): void {
  ...
  toggleFavorite(): void {
    ...
    this.roomService.toggleFavorite(this.roomsId, 'rooms').subscribe({
      next: () => {
        ...
        this.goToDetailsRoom1(this.roomsId); // Re-fetch details to confirm state
      },
      error: (error) => {
        console.warn(
  ...
  goToDetails(roomsId: number): void {
    this.router.navigate(['/details', roomsId]);
  }
  goToDetails1(roomsId: number): void {
    this.router.navigate(['/details', roomsId]).then(() => {
